...
class BD_Anao_Engenheiro_Controller extends BD_AnaoController;
...
if(WorldInfo.TimeSeconds - timeclean > 10)
		    {
				...
				BD_Anao_Engenheiro_Pawn(pawn).CleanAlvos();
		    }
...
//--------------------------------------------------
		...
		//if(BD_AnaoPawn(pawn).alvos.length != 0){
			`log(self@"Tenho"@BD_Anao_Engenheiro_Pawn(pawn).alvos.length@"inimigos");
			proxalvo = BD_Anao_Engenheiro_Pawn(pawn).next();
		//}
		 if(BD_AnaoPawn(pawn).bStun)
		 {
			...
			if(WorldInfo.TimeSeconds - time > stumtime){
				BD_Anao_Engenheiro_Pawn(pawn).bStun = false;
				BD_Anao_Engenheiro_Pawn(pawn).health = BD_Anao_Engenheiro_Pawn(pawn).maxhealth;
			}
		}
         ...
         else
         {
			  ...
			  BD_Anao_Engenheiro_Pawn(pawn).update_maxhealth();

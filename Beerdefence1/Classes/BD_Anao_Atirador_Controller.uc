 ...
 if(WorldInfo.TimeSeconds - timeclean > 10)
		    {
				...
				BD_Anao_Atirador_Pawn(pawn).CleanAlvos();
		    }
...
//--------------------------------------------------
		...
		//if(BD_AnaoPawn(pawn).alvos.length != 0){
			`log(self@"Tenho"@BD_Anao_Atirador_Pawn(pawn).alvos.length@"inimigos");
			proxalvo = BD_Anao_Atirador_Pawn(pawn).next();
		//}
		 if(BD_Anao_Atirador_Pawn(pawn).bStun)
		 {
         	self.Stop();      //Faz o Anão parar seus movimentos

			if(WorldInfo.TimeSeconds - time > stumtime){
				BD_Anao_Atirador_Pawn(pawn).bStun = false;
				BD_Anao_Atirador_Pawn(pawn).health = BD_AnaoPawn(pawn).maxhealth;
			}
		}
         ...
         else
         {
			  ...
			  BD_Anao_Atirador_Pawn(pawn).update_maxhealth();
